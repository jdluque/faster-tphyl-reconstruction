...
if ".false_positives" in filename:
    sim_name = filename.replace(".false_positives", "").replace(
        f"-fp_{fp_probability}", "-fp_0"
    )
else:
...
before_noise_df = pd.read_csv(
    os.path.join(args.original, before_noise_file), sep="\t", header=None
)
...
if os.path.exists(solution_file):  # if this solution file exists
    ...
    for i in range(before_noise_df.shape[1]):  # hypothetical decendant
        for j in range(i + 1, before_noise_df.shape[1]):  # hypothetical ancestor
            if not np.any(
                before_noise_df.iloc[:, i].values.view(bool)
                & ~before_noise_df.iloc[:, j].values.view(bool)
            ):  # Cell that has i but not j
                ancestor_descendant_pair_count += 1
                if not np.any(
                    solution_df.iloc[:, i].values.view(bool)
                    & ~solution_df.iloc[:, j].values.view(bool)
                ):
                    consistent_pair_count += 1

    different_lineage_numerator, different_lineage_denominator = different_lineage(
        solution_df.values, before_noise_df.values
    )
    different_lineage_accuracy = (
        different_lineage_numerator / different_lineage_denominator
        if different_lineage_denominator
        else 1
    )
else:  # Assume that the simulation timed out
    l1_distance = -1
    ancestor_descendant_pair_count = -1
    consistent_pair_count = -1
    ...
    [
        {
            ...
            "different_lineage_numerator": different_lineage_numerator,
            "different_lineage_denominator": different_lineage_denominator,
            "different_lineage_accuracy": different_lineage_accuracy,
            "filename": filename,
